...
공통 규칙
- 병합 후 판정: `../review/CLAUDE_ADJUDICATION.md`와 결정 로그 R1이 이 문서의 이전 예시보다 우선한다. 구현 검증은 `../review/CODEX_FIX_REPORT.md` 및 `CODEX_VERIFY.mjs`를 참조한다.
...
활성화류 `derive.primaryCTA`가 있으면 최상단 라임 행으로 우선하고 기존 실행 런처는 보조로 표시한다. 활성화 선택 카드는 API와 카드가 모두 없을 때만 보이며 버튼은 해당 입력 폼으로 직행한다. 하나라도 완료하면 연결/등록 상태 한 줄로 대체한다. 현재 온보딩 단계는 첫 미완료 단계다. 회원 미활성 뷰에는 preview 링크가 없다.
...
- 병합 수정: 시세·손절·익절·주문가는 `tfTmPx`의 USDT 표기, 규칙 계정 예산과 손익은 원화를 유지한다. 모바일 헤더 아래 운용·누적 손익 요약을 표시한다. 모의 전략 재개는 거래소 연결을 요구하지 않는다.
- 따라가기 계정은 기존 `cpCalc` 원장과 USDT를 유지한 별도 관리 행으로 노출한다. 규칙/복제 VM의 원화 집계와 합산하지 않는다. 따라가기만 소유해도 active 화면이다. 긴급 정지는 규칙/복제 전략을 off로 전환하고 따라가기 계정은 기존 `cpClose` 정산 경로로 종료한다.
...
## S7 전략 따라하기
- 병합 수정: 마켓과 기간 없는 상세의 기본 기간은 `전체`다. 사용자가 고른 기간과 명시적 `/1y`, `/2y` 딥링크는 유지한다. 검증 배지는 실제 데이터의 개월 수를 표시한다. 결측 팔로워 수익은 카드에서 숨기고, 합성하지 않는다. 모바일은 탭 1행과 가로 스크롤 컨트롤 1행, 검색 버튼으로 구성한다.
- `#/share/copy/<nick>`는 부팅 복원 후 상세를 렌더하고 다음 태스크에서 시트를 연다. 활성화 후 같은 닉네임과 입력 예산으로 시트를 복원한다. 입력 예산의 원장은 `cpStart`/`cpClose`가 처리한다.
